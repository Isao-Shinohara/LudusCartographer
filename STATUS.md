...
最終更新: 2026-03-07 (チュートリアル振り返り & リファクタリング)

---

## 現在のフェーズ: メインクエスト自動攻略準備

### 2026-03-07 チュートリアル振り返り & コード永続化

#### 完了項目
- **Post-Mortem 作成**: `crawler/storage/post_mortem.md` — KPT 分析 + RCA テーブル
- **ハードコード座標修正**: 残り 7 箇所を `roi_to_device()` / 比率ベースに統一
  - ご注意画面 agree フォールバック
  - テキスト入力エリア フォールバック
  - 名前入力 OK (Y 座標)
  - 確認ダイアログ OK フォールバック
  ...
  - 規約同意スワイプ座標
  - 通常攻撃ボタン フォールバック
- **Troubleshooting ガイド作成**: `docs/troubleshooting.md`
- **STATUS.md / MEMORY.md 更新**

#### 次フェーズ: メインクエスト自動攻略
| 課題 | 対策 |
|------|------|
| バトル戦略の多様化 | SDE バトル判断ロジック拡張 |
| ダンジョン探索の 3D 操作 | ミニマップ解析 + 方向推定 |
| ポップアップの多様化 | x テンプレート複数化 + OCR 辞書拡充 |
| Wi-Fi 接続不安定 | USB フォールバック自動切替 |

---

### 2026-03-06 最終ミッション成果 (3コミット)

| コミット | 内容 |
|---------|------|
| `e6bf81b` | Phase 2: 環境依存排除・汎用化 (tempfile, pathlib, get_device_resolution) |
| `bcb93b3` | 最終ミッション: ロジック精密化・アナリティクス・Ctrl+C対応 |

#### 実装完了項目
- **ダイアログ誤検出防止**: 枠中心 X を 20%-80% に厳格制限 → ホーム画面設定ボタン誤タップ解消
- **process_paging_dialog()**: ▷ → ▷ → × を一括処理 (#0-DIALOG 統合)
- **アナリティクス**: Dialog/Finger/GoldBtn検知カウント・平均ms/loop・SIGSEGV回避回数
- **戦績サマリー**: Rank 1 / Home Reached / All Tutorials Cleared
- **Ctrl+C シグナルハンドラ**: SIGINT で自動レポート生成・pbcopy

#### 残課題 (将来改善)

| 優先度 | 課題 |
|-------|------|
| 低 | `0%6` 等の OCR誤読をダウンロード進捗と誤認する問題 |
| 低 | ページング▷テンプレート画像の追加 (現状は Canny フォールバックで動作中) |

#### 今セッションで修正したバグ (前セッション分)

| コミット | 修正内容 |
|---------|---------|
| `a01ed29` | バトルResult「次へ」ボタン優先タップ |
| `307c6c4` | GoldSwipe連続ループ脱出 (6回超→OCRフォールバック) |
| `893600f` | detect_and_actのGoldSwipe連続ループも制限 |
| `99354b6` | バトルUI存在時はGoldSwipeをスキップ |
| `e8f7509` | Result「次へ」優先タップ強化 (EXP/Lv.1コンテキスト追加) |
| `5bc71c5` | Result「次へ」位置フィルタ (右下エリアのみ) |
| `f132706` | ADV選択肢「はい」をブロブ検出より優先タップ |
| `3ea73d5` | チュートリアルダイアログ ▷→× ページ送り対応 |
| `36f958d` | FAST_PATHでバトルUI確認済みならGoldSwipeスキップ |
| `99ce627` | GACHA_OKタップ後のwait 0.5s→5.0s (演出完了待ち) |

#### 残課題
- プレゼントボックス「一括受取」のOCR検出: 右下オレンジボタンはOCRで読めないため手動で対処済み (チュートリアル内1回のみ)
- チュートリアルダイアログ ▷→× テンプレート画像: 自動保存済み (次回セッションで活用)
