...
【シミュレータ使用（推奨）】
  ...
  export IOS_USE_SIMULATOR=1          # シミュレータを使う場合

  # シミュレータ UDID を固定する場合（省略時は自動選択）
  export IOS_SIMULATOR_UDID="BA7E719D-8EBA-4049-996C-AC51945A7AE4"  # iPhone 16 iOS 18.5
...
【Vertex AI 解析（任意）】
  export GCP_PROJECT_ID="my-gcp-project"  # 未設定時はスキップ

【MySQL 保存（任意）】
  export DB_HOST=localhost DB_NAME=ludus_cartographer DB_USER=root DB_PASSWORD=secret
...
from lc.capabilities import ios_config_from_env, simulator_config_from_env
from lc.driver import ios_session, ios_simulator_session
...
def main() -> None:
    ...
    # --- デバイス設定（シミュレータ or 実機） ---
    use_simulator = os.environ.get("IOS_USE_SIMULATOR", "").strip() in ("1", "true", "yes")
    # IOS_SIMULATOR_UDID が設定されていれば自動的にシミュレータモードにする
    if os.environ.get("IOS_SIMULATOR_UDID", "").strip():
        use_simulator = True
    ...
    device_type = "Simulator" if use_simulator else "実機"
    logger.info("=" * 60)
    logger.info(f"LudusCartographer — 最小疎通確認 [{device_type}] (Vertex AI + MySQL 統合版)")
    logger.info(f"  UDID     : {cfg.udid}")
    logger.info(f"  Bundle ID: {cfg.bundle_id}")
    logger.info(f"  Device   : {cfg.device_name}")
    if use_simulator:
        logger.info(f"  iOS      : {cfg.platform_version}")
    ...
    session_ctx = ios_simulator_session(cfg) if use_simulator else ios_session(cfg)
    with session_ctx as d:
        # --------------------------------------------------
        # STEP 1: 起動確認
        # --------------------------------------------------
