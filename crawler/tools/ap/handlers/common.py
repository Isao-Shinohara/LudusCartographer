...
def handle_common_guards(ctx: DetectContext, state: PilotState) -> Optional[tuple[str, float]]:
    ...
    # ── 【#-4b】MOVIE→UNKNOWN 遷移直後: 検知は行うがタップを抑制 ──
    # _from_movie_ttl > 0 の間は dispatch をフォールスルーさせ、
    # tap_device 内で実際のタップをスキップする
    state.cycle.tap_suppressed = getattr(state.cycle, "_from_movie_ttl", 0) > 0
    ...
    if state.cycle.download_active:
        ...
        if not state.cycle.home_reached:
            if len(texts) == 0:
    ...
    if _has_download_text and _has_size_progress and not _dl_is_confirm_dialog:
        ...
        # 大規模DL (100MB以上) のみタイムライン記録 (小規模DLは無視)
        _dl_total_mb = 0.0
        for _dt in texts:
            _m_total = re.search(r"/\s*([\d,.]+)\s*MB", _dt)
            if _m_total:
                try:
                    _dl_total_mb = float(_m_total.group(1).replace(",", ""))
                except ValueError:
                    pass
                break
            _m_gb = re.search(r"/\s*([\d,.]+)\s*GB", _dt)
            if _m_gb:
                try:
                    _dl_total_mb = float(_m_gb.group(1).replace(",", "")) * 1024
                except ValueError:
                    pass
                break
        if "DL_START" not in state.cycle.milestone_logged:
    ...
    if _dl_is_failure and _has_download_text:
        state.cycle.download_active = False
        _ok_x, _ok_y = roi_to_device(int(ANALYSIS_W * 0.62), int(ANALYSIS_H * 0.82), state.cycle.game_roi)
