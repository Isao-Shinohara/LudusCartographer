...
- 人物（キャラクター）の本体が**明瞭に**視認できる画面（全身や顔だけでなく、手元・目元・口元などの「身体の一部のクローズアップ」や「後ろ姿」もすべて含む。テキストが一切ないアニメ風のカットも必ず false）
  ※注意1: 画面下部の「小さなキャラの顔アイコン」だけが見えている状態は、キャラクターが描かれているとは見なさない
  ※注意2: 白飛びや暗転の途中で、キャラクターがうっすらとしたシルエットしか見えない状態はここには含めない
- メニュー、ホーム、編成、バトルコマンド選択（画面がエフェクトで覆われていない安定した状態に限る）、ダイアログ、情報が完全に表示され終わったリザルト画面
- セリフ付きの会話シーン（ADV）
...
2. **暗転・黒ベタ**: 画面がほぼ黒一色、またはロード中を示すアイコンしかない画面
3. **ホワイトアウト・白飛び**: 画面の大部分（70%以上）が白またはごく淡い色で覆われ、以下のいずれかに該当する場合:
   - キャラクターやUIが白い膜の下にうっすらとしか見えない（演出の途中フレーム）
   - 画面がほぼ白一色で、小さなテキストラベルが少量あるだけで構造的なUIが存在しない（報酬獲得演出の途中フレーム等）
4. **不完全なキャプチャ**: 画面が半分切れている、または著しいノイズで状況が不明

**判定の原則**: 人物の本体やその身体の一部が**明瞭に**描かれているなら false。迷ったら false。
