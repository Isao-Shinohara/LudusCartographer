...
## キャプチャ不良の検出 (is_artifact)
画面キャプチャが正常に取れていない場合のみ true を返す:
- 右端 / 左端 / 上端 / 下端に明らかな黒帯・色帯があり、画面がずれている
- Android のステータスバー / ナビゲーションバーが画面に被っている
- スクショ全体が著しく歪んでいる、ピクセルが乱れている

エフェクト・暗転・動画シーン等の「正常なゲーム画面」は false にする。
判定が曖昧な場合は false (誤除外を防ぐため)。

各 result オブジェクトに以下を追加:
"is_artifact": false  // または true (キャプチャ不良時のみ)'''
...
# ── クラスタリングルール (ローカル dedup と Gemini 判定で共有) ──
# CLAUDE.md §16 と同期すること
_CLUSTERING_RULES = """
## 「同じ画面」の判定基準（最優先: テキスト）
- テキスト（セリフ・UI文字）が同一、または一方が他方の先頭部分（前方一致）なら同じ画面
- テキストがない画像同士で、見た目がほぼ同じなら同じ画面
- テキストがない画像とテキストがある画像で、背景が同じなら同じ画面（テキスト追加前後）
- カーソル位置・選択ハイライトの違いはOK
- 動画の連続フレーム（背景の動きはOK、内容が同じ場面）

## 「別の画面」とすべきもの（厳格）
- テキスト（セリフ）が異なる場合は、背景が同じでも別の画面（最重要ルール）
- 別のメニュー / 別のシーン
- 異なる選択肢を表示しているダイアログ
- 内容が大きく異なる動画シーン
""".strip()

_GROUPING_PROMPT = f'''最初の画像（画像 1）はアンカー画面です。
