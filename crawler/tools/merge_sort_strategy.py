...
class SafeInsertStrategy(MergeSortStrategy):
    ...
    def preview_insertable(
        ...
        # session_fp → master_fp (マッチしたものは既存 master_fp、新規は session_fp 自身)
        all_mappings: dict[str, str] = {}
        for s_fp, (m_fp, method, score) in node_mapping.items():
            all_mappings[s_fp] = m_fp
        # 新規ノードは session_fp をそのまま master_fp として扱う
        session_reps = conn.execute(
            "SELECT fingerprint FROM lc_screens"
            " WHERE session_id = ? AND is_representative = 1"
        ...
        for r in session_reps:
            fp = r["fingerprint"]
            if fp not in node_mapping:
                all_mappings[fp] = fp  # 新規: session_fp を master_fp として使う
                new_fps.add(fp)
        ...
        # アンカー列を構築
        anchor_sequence = self._build_anchor_sequence(
            session_reps, all_mappings, new_fps, sort_orders
        )
        ...
        for screen in session_reps:
            s_fp = screen["fingerprint"]
            m_fp = all_mappings.get(s_fp)
            if not m_fp:
                continue

            if m_fp in new_fps and m_fp not in seen_new:
                pending_new.append(s_fp)
                seen_new.add(m_fp)
            elif m_fp in sort_orders and m_fp not in new_fps:
                cur_anchor_sort = sort_orders[m_fp]
                if pending_new:
