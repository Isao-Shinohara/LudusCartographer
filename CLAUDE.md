...
詳細仕様は `docs/` 配下に分離。本ファイルは毎ターン参照する規範 — §11 (禁止) / §13 (行動) / §15 (判断基準) — を厚めに、それ以外は要点 + リンク。
...
実機検証・クローラー開発は必ず最小単位で進めること (コード修正一般のユーザー確認ゲートは §13 A 参照):
...
1. グルーピング
2. PaddleOCR 再処理
3. クラスタリング (OCR 完了後の HQ テキストで比較)
4. 遷移グラフ構築

- 順序は固定。クラスタリングは OCR 後に走らせる必要があるため逆転禁止
- 各ワーカーの実行間隔は `background_worker.py` を参照 (実装値、調整可)
...
隣接アンカーが先頭・末尾・連続 (sort_order 差 = 1) の場合のみ挿入。それ以外は **挿入しない (破棄)**。

**詳細** (条件マトリクス・実装ルール): `docs/merge_sort_algorithm.md`。
