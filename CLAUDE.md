...
- 特に §11（禁止事項）と §13（本セクション）を確認し、ルールに矛盾する変更を行わない
...
### 起動コマンド対応表（厳格・`-r` 関連判断の Single Source of Truth）
...
- **エラー時のキャッシュ禁止（厳格）**: 失敗結果を成功結果と同じ key でキャッシュしない。`error: True` フラグでスキップ
