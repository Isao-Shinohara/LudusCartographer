...
## 画面同一性判定：Title + OCR Content Hash 方式 (Step 5 — 2026-03-03 追加)

### コンテキスト

Phase 3〜4 の `_screen_fingerprint()` は OCR テキストをそのまま MD5 化していた。

問題:
1. **数値の揺らぎ**: 時刻 (`10:30` → `10:31`)・所持金・残弾数・レベル等がフレームごとに変わると、
   同一画面が毎回異なる指紋を持ち「別画面」と誤認識されてしまう。
2. **タイトル衝突**: タイトルが同じでも中身が異なる画面（異なるキャラクターの
   ステータス画面など）を `fingerprint` だけでは区別できなかった。

### 決定 — `_generate_fingerprint` + `{title}@{fingerprint}` キー方式

**ハッシュ前に数字を除去し、_visited の管理キーを `{title}@{fingerprint}` 形式に変更する。**

```
_generate_fingerprint アルゴリズム:
  1. OCR bounding-box の y 座標から screen_h を動的推定
  2. ステータスバー (y < screen_h × 5%) を除外
  3. 信頼スコア < 0.75 を除外
  4. 各テキストに re.sub(r'\d+', '') を適用 → 数字列を空文字に置換
  5. 置換後に 2 文字以上残るテキストのみを採用
  6. ソートして "|" 結合 → MD5 hex 先頭 16 文字を返す

_visited キー:
  "{record.title}@{record.fingerprint}"
```
...
**`{title}@{fingerprint}` キーの効果:**

| ケース | title | fingerprint | キー | 判定 |
|-------|-------|-------------|------|------|
| 同一タイトル・同一内容 | `"HP"` | `"abc"` | `"HP@abc"` | ✅ 同一画面 |
| 同一タイトル・異なる内容 | `"HP"` | `"xyz"` | `"HP@xyz"` | ✅ 別画面 |
| 数値変化のみ | `"HP"` | `"abc"` | `"HP@abc"` | ✅ 同一画面（数値除去で指紋安定） |

### 却下した代替案

| 案 | 却下理由 |
|----|---------|
| 数値を定数 `#` に置換 | 置換後テキストが意味を持ちすぎて誤マッチが増える |
| title のみをキーにする | 同一タイトルの異なる内容画面を区別できない |
| phash のみで同一性判定 | 白系 iOS 画面では距離 2〜4 で全画面が同一視される（既知問題） |

### 影響範囲 (Step 5)

- `crawler/lc/crawler.py` — `_generate_fingerprint()` 追加、`_DIGIT_RE` 定数追加
- `_crawl_impl` — `_visited` キーを `"{title}@{fingerprint}"` 形式に変更
- `ScreenRecord.fingerprint` フィールドと `parent_fp` は変更なし（後方互換）
- `_screen_fingerprint()` は削除せず残存（外部参照の互換性のため）
