...
533枚のゲームスクショで検証した結果:
- `lbpcascade_animeface`: 196枚検出 (36.8%)、平均 8.8ms
- `animeface` (pip): 124枚検出 (23.3%)、平均 62ms
- animeface でしか検出できない画面: 14枚 (2.6%)、うちテキストなしは 4枚 (0.75%)
- **結論**: lbpcascade 単体で十分。精度不足が判明したら animeface を追加検討
...
OCR テキスト正規化:
  ...
  ├── 日本語/英単語を含むトークンのみ
  ├── 純粋数字・時刻パターン除外
  ├── テキスト内の数字も除去 ("Turn 3" → "Turn")
  ├── (center_y // 50, center_x) でソート
  └── テキストのみで | 結合 (座標はハッシュに含めない)
```
...
- **2段階方式**: リアルタイムは寛容に撮影、バッチで間引き・分類・OCR再処理
- **リアルタイム保存条件**: 暗転（輝度 <= 30）と LOADING/MOVIE 以外は全部保存
- **重複排除**: fingerprint（テキストあり→SHA-256、なし→phash）で排除
- **バッチ処理** (`batch_processor.py`):
  - Phase 1: 時系列 + scene でグルーピング + ラベル付け（「バトル#1」「ストーリー#2」等）
  - Phase 2: phash クラスタリングで類似フレーム間引き → 代表1枚 (is_representative)
  - Phase 3: PaddleOCR フル解像度で代表画像の OCR 再処理（テキスト精度向上）
  - Phase 4: CLIP (ONNX) で画面カテゴリ自動分類・タグ付け（将来）
  - 全てローカル完結（従量課金 API は使わない）
  - グルーピング: 同じ scene でも 60秒以上の空白があれば別グループ
- **対象**: ゲーム起動後のみ。インストール画面・Play Store はスクショ対象外
- **座標はソート用のみ**: ハッシュに座標を含めない（アニメーション揺れ対策）
- **数字除外**: ターン番号・ダメージ値・タイマー等の動的数値を無視
- **記号除外**: OCR ノイズ（`♦`, `●`, `▶` 等）を無視
- **方式選択の経緯**: 安定フレーム方式 → 取りこぼし発生 → 寛容撮影+バッチ間引きに変更
